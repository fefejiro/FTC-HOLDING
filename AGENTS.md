# AGENTS.md — FTC HOLDING Monorepo

Last audited: 2026-03-26

This file is the orientation doc for every Codex session in this repo.
...
Known pattern: complex stacks drift; prefers thin, solid infrastructure with Codex as the
...
└── tmp-bin/             ← local binaries: cloudflared.exe, Codex-wrapper*.exe
```
...
- `APPS/ATEAM/tmpclaude-*` — 70+ Codex worktree temp dirs. Safe to delete.
- `APPS/ATEAM/Server/tmpclaude-*` — 24+ more. Safe to delete.
- These are leftover from Codex isolated worktree sessions.
...
- Codex should be the **orchestrator/brain**, not buried inside app logic
- Preferred phone interface: Codex mobile (open-ended audio input) → bridge → PC execution
- Current blocker: Codex mobile is a closed system — no custom server routing
