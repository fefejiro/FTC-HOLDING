...
Generated: 2026-06-16T01:17:49.139Z
Root: `C:\FTC HOLDING`
...
- P0 Railway account: Fix Railway account billing/token authorization before attempting Railway service or domain changes. Evidence: ERRORS: [{"message":"Not Authorized","locations":[{"line":1,"column":9}],"path":["me"],"extensions":{"code":"INTERNAL_SERVER_ERROR"},"traceId":"9002541175992432044"}]
...
- P1 Local app owners: Resolve failing local build/test gates before calling the corresponding app locally healthy. Evidence: saywetin npm run check: Command exited 2. \| dispatch npm run check: Command exited 2.
...
| PASS | gardencleaners-site npm run build | Command completed. |
| PASS | capsigma-growth-desk npm run check | Command completed. |
| PASS | peacepad npm run check | Command completed. |
| PASS | peacepad npm run build:frontend | Command completed. |
| PASS | peacepad npm run build:api | Command completed. |
| FAIL | saywetin npm run check | Command exited 2. |
| PASS | saywetin npm run build:prod | Command completed. |
| FAIL | dispatch npm run check | Command exited 2. |
| PASS | dispatch npm run build | Command completed. |
