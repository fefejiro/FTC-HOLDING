...
As of 2026-04-02, Dispatch is live-only:
...
At the time of this handover, Dispatch was being positioned as an Ottawa roadside assistance product with:
...
- `dispatch.unalabs.cloud` is the live product surface.
...
Dispatch backend deploys from `APPS/dispatch` through the linked Railway service. Push `main`; do not use duplicate root-path deployment commands.
