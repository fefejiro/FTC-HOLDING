# Docs Index

Canonical repo root: `C:\FTC HOLDING`

This index organizes the highest-signal documentation currently in the repo without moving existing files.

## Strategic Architecture

- [Business Alignment Map](BUSINESS_ALIGNMENT_MAP.md)
- [Project Classification Matrix](PROJECT_CLASSIFICATION_MATRIX.md)
- [Unalabs Ecosystem Map](UNALABS_ECOSYSTEM_MAP.md)
- [Repo Structure Recommendation](REPO_STRUCTURE_RECOMMENDATION.md)
- [Domain and Ownership Map](DOMAIN_AND_OWNERSHIP_MAP.md)
- [Unalabs Rollout Plan](ROLLOUT_PLAN_UNALABS.md)
- [Repo Ownership and Tracking](REPO_OWNERSHIP_AND_TRACKING.md)
- [Repo Handover Audit 2026-03-07](REPO_HANDOVER_AUDIT_2026-03-07.md)

## Operator Clarity

- [New Employee / Operator Guide](NEW_EMPLOYEE_OPERATOR_GUIDE.md)
- [Git and Publish Path Guide](GIT_AND_PUBLISH_PATH_GUIDE.md)
- [Founder Dashboard](FOUNDER_DASHBOARD.md)
- [Founder Dashboard Quickstart](FOUNDER_DASHBOARD_QUICKSTART.md)
- [Idea Triage Matrix](IDEA_TRIAGE_MATRIX.md)
- [Idea Triage Quickstart](IDEA_TRIAGE_QUICKSTART.md)
- [Dual-Lane Tracker](DUAL_LANE_TRACKER.md)
- [Dual-Lane Tracker Quickstart](DUAL_LANE_TRACKER_QUICKSTART.md)

## Operations And Runbooks

- [Runbook](RUNBOOK.md)
- [Cloudflare Pages Setup](CLOUDFLARE_PAGES_SETUP.md)
- [Railway Setup](RAILWAY_SETUP.md)
- [Deploy Commands](DEPLOY_COMMANDS.md)
- [Security Rotation Checklist](SECURITY_ROTATION_CHECKLIST.md)

## PeacePad

- [PeacePad Release Policy](PEACEPAD_RELEASE_POLICY.md)
- [PeacePad Pages Setup](PEACEPAD_PAGES_SETUP.md)
- [PeacePad Railway API Setup](PEACEPAD_RAILWAY_API_SETUP.md)
- [PeacePad Capability Productization](PEACEPAD_CAPABILITY_PRODUCTIZATION.md)
- [PeacePad Capability Audit 2026-03-10](PEACEPAD_CAPABILITY_AUDIT_2026-03-10.md)
- [PeacePad Android Release Guide](PEACEPAD_ANDROID_RELEASE.md)

## Saywetin

- [Saywetin Handover](SAYWETIN_HANDOVER.md)
- [Saywetin Release Policy](SAYWETIN_RELEASE_POLICY.md)
- [Saywetin Split Deploy Runbook](SAYWETIN_SPLIT_DEPLOY_RUNBOOK.md)
- [Saywetin Android Release Guide](SAYWETIN_ANDROID_RELEASE.md)

## Umbrella Site / Unalabs Site

- [Una Labs Site Handover 2026-03-10](UNALABS_SITE_HANDOVER_2026-03-10.md)
- [LinkedIn Positioning Update 2026-03-19](..\APPS\ftc-site\docs\LINKEDIN_POSITIONING_UPDATE_2026-03-19.md)

## Working Rule

Use this index as a navigation layer. Do not treat it as a signal that the repo has already been physically reorganized.
