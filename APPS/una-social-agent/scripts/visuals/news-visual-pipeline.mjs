...
export function createVisualBrief(facts) {
  ...
  return {
    ...
    generation_prompt: buildEditorialImagePrompt({
      headline: facts.headline,
      primarySubject: facts.primary_subject,
      subjectAction: subjectAction,
      environment: facts.physical_environment,
      technologySignals: facts.technology,
      regionalContext,
      mustShow: unique([facts.primary_subject, ...facts.technology.slice(0, 3), subjectAction, ...supporting.slice(0, 3)]),
      mustNotShow,
...
export function evaluateImageRelevance(facts, brief, assetMeta = {}) {
  const prompt = [
    facts.headline,
    facts.primary_subject,
    facts.physical_environment,
    facts.technology.join(' '),
    facts.visible_objects.join(' '),
    facts.important_actions.join(' '),
    facts.real_world_application.join(' '),
    brief.supporting_elements.join(' '),
    brief.must_show.join(' '),
  ].join(' ').toLowerCase()
...
export function slideHtml({ facts, brief, imageDataUrl, index, sourceName }) {
  const headline = sanitizeHeadline(facts.headline, { maxWords: 10, maxChars: 68 }).toUpperCase()
  const summary = sanitizeDeck(facts.plain_language_summary || facts.real_world_application.join(', '), { maxWords: 18, maxChars: 150 })
  const consequence = sanitizeDeck(
    facts.real_world_application.length > 1
      ? `${facts.real_world_application.join(', ')} now needs clearer ownership and review.`
      : brief.editorial_angle,
    { maxWords: 18, maxChars: 135 },
  )
  return `<!doctype html><html><head><meta charset="utf-8"/><style>
    *{box-sizing:border-box} html,body{margin:0;width:1080px;height:1350px;overflow:hidden;background:#f47f13;font-family:Arial,Helvetica,sans-serif}
    ...
    <div class="body">
      ...
      <p class="why">${escapeHtml(consequence)}.</p>
    </div>
