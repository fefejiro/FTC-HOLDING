...
| Gate 1: Canada/U.S. platform | 15% | 80% | HOSTED VERIFIED / DEPLOYMENT BLOCKED | Both regional boundary schemas are deployed. Hosted CI typechecks the Edge Function, applies every migration twice, and executes the PostgreSQL transaction proof through persisted calendars. The current CLI identity still cannot update Function secrets or deploy. Managed restoration and residency assurance remain absent | Grant the CLI identity Owner/Administrator project access, apply pending migrations through `202608090005`, deploy both adapters, and run live contract checks |
| Gate 2: identity and coordination | 20% | 50% | HOSTED VERIFIED / DEPLOYMENT BLOCKED | Identity, consent, invitation lifecycle, deletion, messaging history, calendar layers, explicit sharing, schedule events, idempotency, concurrency, audit, and cross-region denial pass hosted PostgreSQL proof. Remote persistence and two-account device proof are absent | Deploy the current regional adapter, then complete the fictional two-account invitation, messaging, and calendar journey |
...
Weighted implementation estimate: **about 34% production-ready**. This replaces
