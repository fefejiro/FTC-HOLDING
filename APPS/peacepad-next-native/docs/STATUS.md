...
unavailable, so no production build 5 binary, App Store attachment, review submission, or
...
1. **Internal TestFlight proof:** the exact `2.0.0` (`3`) build is processed `VALID` and assigned to the existing 2-person internal group. Each tester must now install it from TestFlight on physical hardware using only dedicated fictional accounts. Capture the build number, device/iOS version, staging-region label, first-launch result, and cleanup result.
...
D:-backed launcher verified the exact native Git tree, uploaded a 1.8 MB
app-only archive, and passed the configured remote credential selection for
`ca.peacepad.family`; EAS then stopped before cloud build creation with its
free-plan quota message (reset 2026-09-01). As the quota-free alternative,
GitHub run [`31642371405`](https://github.com/fefejiro/FTC-HOLDING/actions/runs/31642371405)
on a standard hosted macOS runner checked out exact source
`710697817aa2666b7ef7b13ef711aa563976da54`, built the IPA locally with the
`testflight-internal` profile, and uploaded a 25,041,566-byte signed candidate.
Its independently rechecked SHA-256 is
`b863d431e9e259b9b41129ca6598082148a44186e86ccf4bbe0b34179f7bd2ec`;
the artifact contains bundle `ca.peacepad.family` and an iOS
`_CodeSignature/CodeResources` manifest. The hosted evidence manifest records
source tree `39e85e97daa41d4a77ed2f4a36560d94152551c7` and
`submission_attempted=false`. The profile remains fictional staging with
...
finished on 2026-08-13. App Store Connect processed exact build resource
`4c4b3b9e-b7f5-4da9-ab80-66ad8d6e2091` as `VALID` and assigned it to the
existing 2-person `PeacePad Internal` group. Physical installation, external
beta review, App Store review, and public release remain unverified.
...
| Gate 6: migration and App Store | 15% | 45% | INTERNAL TESTFLIGHT AVAILABLE + D:-BACKED ONE-ARTIFACT DUAL-REGION SIMULATOR VERIFIED / DEVICE, MIGRATION, AND RELEASE GATES BLOCKED | Gates 0-5 are incomplete. Exact source `2fe8ca82b` produced one hash-verified EAS Simulator artifact through an app-only D:-backed upload; protected runs `31554747960` and `31557228680` selected Canada and U.S. respectively from that identical artifact and proved authenticated native messaging/Calendar coordination across two fictional accounts per region plus complete cleanup. Exact source `ec45198f1bdbd849121ab330cbdba0c53a95aeec` produced signed PeacePad `2.0.0` build `3` for existing bundle `ca.peacepad.family` and Apple ID `6793350735`. App Store Connect processed build resource `4c4b3b9e-b7f5-4da9-ab80-66ad8d6e2091` as `VALID` and assigned it to the existing 2-person internal group. The candidate still uses fictional staging with production writes disabled. No physical-device install, external beta review, migration, public App Store review, or production cutover proof exists | Install exact build 3 through internal TestFlight on physical devices, complete migration/rollback and remaining device journeys, soak, sign-offs, create the reviewed 2.0 public version only after the real production boundary is ready, and submit to App Review |
...
| Migration, TestFlight, and release | 45% | INTERNAL TESTFLIGHT AVAILABLE + D:-BACKED ONE-ARTIFACT DUAL-REGION SIMULATOR VERIFIED / DEVICE AND RELEASE GATES BLOCKED | Exact source `2fe8ca82b` packages the clean native app Git tree on D:, avoids copying dependencies or cloning the multi-gigabyte monorepo, and produced one exact Simulator artifact that passed authenticated Canada and U.S. coordination journeys. Signed version `2.0.0` (`3`) for existing bundle `ca.peacepad.family` was processed `VALID` by App Store Connect and is assigned to the existing 2-person internal group. It retains fictional dual-region staging, disabled diagnostics, and disabled production writes. No physical-device install, migration, external beta review, public App Store review, or production proof exists. MacinCloud is past due and optional; GitHub macOS plus App Store Connect is the active path |
...
| Apple credentials and quota-free signed IPA | `710697817aa2666b7ef7b13ef711aa563976da54`; main control `b7958892602893e626201d65ca8a6dac687346d6` | Windows local EAS CLI, Apple Developer portal, and standard GitHub-hosted macOS | 2026-08-12 | APPLE CREDENTIALS + SIGNED IPA VERIFIED / TESTFLIGHT NOT SUBMITTED | The approved `official_fejiro` EAS project has the active Apple distribution certificate and provisioning profile for bundle `ca.peacepad.family` under team `G6UNC88GQ5`; EAS accepted the 1.8 MB app-only upload but its free cloud build allocation remains exhausted until 2026-09-01. The standard hosted-macOS control avoids that EAS build quota: [run `31642371405`](https://github.com/fefejiro/FTC-HOLDING/actions/runs/31642371405) bound the exact source, built a 25,041,566-byte `testflight-internal` IPA locally, and uploaded evidence. The locally rechecked artifact SHA-256 is `b863d431e9e259b9b41129ca6598082148a44186e86ccf4bbe0b34179f7bd2ec`; the manifest records source tree `39e85e97daa41d4a77ed2f4a36560d94152551c7`, `submission_attempted=false`, and the artifact contains `ca.peacepad.family` plus `_CodeSignature/CodeResources`. This is not a TestFlight upload, submit, device, or production proof. The earlier static preflight source `25c9a6358` remains HOSTED VERIFIED in [native run `31611086473`](https://github.com/fefejiro/FTC-HOLDING/actions/runs/31611086473) and [infrastructure run `31611087586`](https://github.com/fefejiro/FTC-HOLDING/actions/runs/31611087586). |
| Xcode 26 TestFlight submission | `82fad8f87b440215d681f4e6820e205ca8abfe1b`; main control `78b18c0c78197f7f891d341692bfc79d771523f5` | GitHub-hosted macOS 26, EAS Submit, App Store Connect | 2026-08-13 | SIGNED IPA VERIFIED / SUBMISSION IN_QUEUE / TESTFLIGHT PROCESSING PENDING | Run [31647572665](https://github.com/fefejiro/FTC-HOLDING/actions/runs/31647572665) built the 25,118,188-byte IPA with Xcode 26.6 (17F113). SHA-256 is `28913054e9884fe74e2b1b6bb72aaec8b0348ca2f327c39b02ec5709dcbbdeca`. Submission [644d93e8-224b-46f4-aa30-4c95b973dc15](https://expo.dev/accounts/official_fejiro/projects/peacepad-next-native-lab/submissions/644d93e8-224b-46f4-aa30-4c95b973dc15) was accepted with the configured Apple API key and remains `IN_QUEUE`; no TestFlight processing, install, review, or public release is claimed. |
