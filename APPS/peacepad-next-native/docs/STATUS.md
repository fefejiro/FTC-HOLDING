...
**BLOCKED — Gates 0 and the current fictional database contract are HOSTED VERIFIED; regional API deployment remains blocked by project role.**
...
| Gate 1: Canada/U.S. platform | 15% | 76% | HOSTED VERIFIED / DEPLOYMENT BLOCKED | Both regional boundary schemas are deployed. Hosted CI now typechecks the Edge Function, applies every migration twice, and executes the PostgreSQL transaction proof. The current CLI identity still cannot update Function secrets or deploy. Managed restoration and residency assurance remain absent | Grant the CLI identity Owner/Administrator project access, apply pending migrations through `202608090004`, deploy both adapters, and run live contract checks |
| Gate 2: identity and coordination | 20% | 44% | HOSTED VERIFIED / DEPLOYMENT BLOCKED | Identity, consent, complete invitation lifecycle, account deletion, conversations, immutable messages/corrections, delivery/view receipts, and effective-content search pass hosted PostgreSQL proof. Remote persistence and two-account proof are absent | Deploy the current regional adapter, then complete the fictional two-account invitation and messaging journey |
...
Weighted implementation estimate: **about 31% production-ready**. This replaces
...
| Messaging and Message Check | 72% | HOSTED VERIFIED | Regional conversations, immutable messages/corrections, delivery/view receipts, idempotency, search, authorization, and audit pass executable PostgreSQL proof. Message Check remains client/rule based and deployed delivery is unverified |
