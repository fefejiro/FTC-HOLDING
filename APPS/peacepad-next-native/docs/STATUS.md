...
- Verified implementation baseline: `600ce2e59`
...
**BLOCKED - Gate 0, the fictional database contract, authenticated native runtime, and family onboarding are HOSTED VERIFIED; the regional deployment control is registered and protected, but managed deployment still lacks the required Supabase access token and regional database URLs.**
...
infrastructure workflows pass for the current documentation baseline
`600ce2e59` (native run `31346107125`; infrastructure run `31346107130`). The
overall PR remains unstable only because the unrelated Garden workflow cannot
load `@ftc/config/dist/index.js`. PR #178 registered the main-controlled
...
environments still lack `SUPABASE_ACCESS_TOKEN` and `DATABASE_URL`.
...
| Gate 1: Canada/U.S. platform | 15% | 80% | HOSTED VERIFIED / DEPLOYMENT BLOCKED | The initial regional boundary schemas are deployed. Migrations `202608090001` through `202608090011` and the Edge adapter are not managed-project verified. Hosted CI typechecks the Edge Function, applies every migration twice, and executes the PostgreSQL transaction proof through persisted calendars and Message Check. The main-controlled deployment workflow and protected regional environments are ready, and maintenance secrets are configured. The environments still lack `SUPABASE_ACCESS_TOKEN` and `DATABASE_URL`; the current local CLI identity cannot see either project. Managed restoration and residency assurance remain absent | Add the four missing environment-secret slots privately, dispatch a Canadian dry run and then a U.S. dry run from `main`, review their evidence, deploy one region at a time, and run live contract checks |
...
| Messaging and Message Check | 84% | HOSTED VERIFIED | Regional conversations, immutable messages/corrections, delivery/view receipts, idempotency, search, persisted per-identity/per-conversation Message Check preferences, default-off behavior, explicit opt-in/out, rule-based preview authorization, original-draft preservation, AI-consent separation, authorization, and audit pass executable PostgreSQL proof. The bounded outbox preserves original and selected drafts, keeps terminal failures visible as needs-action, prevents cross-session automatic send, and supports exact-scope explicit retry/removal. Managed deployment and two-device delivery remain unverified |
...
| Offline synchronization | 28% | HOSTED VERIFIED | SecureStore queue is capped at 5 messages / 800 characters, exactly scoped to identity, auth session, region, family, and conversation, preserves idempotency and original drafts, retries once per verified runtime hydration, retains terminal/auth/conflict/exhausted entries as needs-action, supports explicit exact-scope retry or confirmed device-local removal, reconciles canonical messages after ambiguous timeouts, and clears before sign-out or after verified account deletion. No SQLite cache, network-triggered retry, Simulator, or real-device proof exists |
...
| Current PR #177 scoped gates | `600ce2e59` | GitHub Actions, PR #177 | 2026-08-09 | HOSTED VERIFIED / UNRELATED MONOREPO CHECK FAILED | Native run `31346107125` and infrastructure run `31346107130` passed at this exact head. The overall PR remains unstable only because Garden run `31346107131` cannot load `@ftc/config/dist/index.js`; no Garden code was changed |
| Registered regional deployment control | `2feaf1f3d` | GitHub Actions and GitHub environments, `main` | 2026-08-09 | HOSTED VERIFIED / DEPLOYMENT CREDENTIALS BLOCKED | PR #178 merged; static run `31347432290` passed. The workflow binds its control checkout to the dispatch commit and its detached target checkout to exact `600ce2e59`, defaults to dry-run, exposes one region at a time, and keeps secrets step-scoped. Both environments are `main`-only with required reviewer `fefejiro`; distinct maintenance secrets are configured. `SUPABASE_ACCESS_TOKEN` and `DATABASE_URL` remain absent in each environment |
