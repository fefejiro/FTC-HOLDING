...
| Gate 1: Canada/U.S. platform | 15% | 65% | DUAL-REGION SUPABASE STAGING VERIFIED | Both regional boundary schemas are deployed; API adapters, managed restoration, and residency assurance remain absent | Deploy the Canadian `/api/v2` adapter, repeat in the U.S., then prove restoration |
