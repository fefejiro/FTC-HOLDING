...
- Verified implementation baseline: `d2a98075a`
...
infrastructure workflows pass for the current offline-recovery implementation
`d2a98075a` (native run `31343203497`; infrastructure run `31343203496`). The overall PR remains unstable
...
| Gate 4: calls, offline, parity | 15% | 5% | HOSTED VERIFIED / INCOMPLETE | A same-session, identity/region/family/conversation-scoped SecureStore message outbox now retains transient and needs-action intents without automatic cross-session replay. It is deliberately bounded: no SQLite durable cache, network listener, manual conflict UI, device proof, or calling service | Prove queued-message recovery on Simulator and two fictional real-device accounts before adding SQLite or calls |
...
Weighted implementation estimate: **about 40% production-ready**. This replaces
...
| Messaging and Message Check | 82% | HOSTED VERIFIED / LOCAL OFFLINE VERIFIED | Regional conversations, immutable messages/corrections, delivery/view receipts, idempotency, search, persisted per-identity/per-conversation Message Check preferences, default-off behavior, explicit opt-in/out, rule-based preview authorization, original-draft preservation, AI-consent separation, authorization, and audit pass executable PostgreSQL proof. The bounded outbox preserves original and selected drafts, keeps terminal failures visible as needs-action, and prevents cross-session automatic send. Managed deployment and two-device delivery remain unverified |
...
| Offline synchronization | 20% | HOSTED VERIFIED | SecureStore queue is capped at 5 messages / 800 characters, exactly scoped to identity, auth session, region, family, and conversation, preserves idempotency and original drafts, retries once per verified runtime hydration, retains terminal/auth/conflict/exhausted entries as needs-action, and clears before sign-out or after verified account deletion. No SQLite cache, network-triggered retry, manual retry/conflict workflow, Simulator, or real-device proof exists |
