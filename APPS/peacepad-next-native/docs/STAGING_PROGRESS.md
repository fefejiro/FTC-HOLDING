...
| Regional request-bound idempotency secret posture | CONFIGURED at `2026-08-10T18:45:45Z`: both protected regional `IDEMPOTENCY_SECRET` slots were securely configured without reading or recording their values. `SUPABASE_ACCESS_TOKEN` and `DATABASE_URL` remain absent in each environment, and the current authenticated Supabase CLI identity cannot see either PeacePad project; therefore managed deployment and live regional proof remain blocked |
