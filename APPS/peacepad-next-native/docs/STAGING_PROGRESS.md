...
| Supabase persisted messaging, calendar, and Message Check transactions | MANAGED message send/deliver/view and replay/conflict VERIFIED in both regions; Calendar and Message Check managed journeys pending |
...
| Regional managed deployment | DEPLOYED STAGING VERIFIED with regional pinning at `df6ed38de`/`2c1c4a308`: hosted gates `31427335593`/`31427335775` and `31427956926`/`31427956997`; protected dry-runs `31427516196`/`31427518536`/`31428115334`; deployments `31427636319` Canada and `31428209000` U.S. All 19 migrations, `peacepad-v2-api`, public boundaries, and the bounded managed two-account contract pass. Restoration remains pending |
| Managed dual-region fictional two-account contract | MANAGED API JOURNEY VERIFIED independently in Canada and the U.S.: Auth, identity bootstrap, wrong-region denial, replay/conflict, invitation acceptance, message delivery/view, call create/accept/end, post-end signal denial, deletion, and old-token denial passed. Temporary accounts were removed; credentials, identity IDs, and message bodies were not logged. This is not complete feature, media, Simulator, device, TestFlight, or production proof |
