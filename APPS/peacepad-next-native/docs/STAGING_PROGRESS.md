...
| Supabase regional API adapters | DEPLOYED STAGING VERIFIED in Canada run `31424695462` and U.S. run `31424817202`; authenticated journeys pending |
| Supabase atomic identity/consent/family/invitation transactions | DEPLOYED STAGING VERIFIED for schema/API surface; live fictional two-account proof pending |
| Supabase persisted messaging, calendar, and Message Check transactions | DEPLOYED STAGING VERIFIED for schema/API surface; live fictional two-account proof pending |
| Authenticated Supabase native runtime and family onboarding | DEPLOYED STAGING VERIFIED for backend surface; managed runtime journey pending |
| Versioned account deletion and local session invalidation | DEPLOYED STAGING VERIFIED for backend surface; live deletion proof pending |
...
| Owner-private source-linked timeline | DEPLOYED STAGING + HOSTED + POSTGRES VERIFIED: the migration is deployed in both replacement regions; earlier proof at `4fa7ba672` covered source authorization, minimization, idempotency, owner isolation, and cleanup. Live managed authorization remains pending; no file bytes, arbitrary narrative, legal conclusion, or export capability was added |
| Secure foreground audio-call lifecycle | HOSTED + POSTGRES VERIFIED at `2a8aac813`: all 18 migrations applied twice locally on disposable PostgreSQL 18.3; complete and focused fictional transaction proofs passed. [Infrastructure run `31419736829`](https://github.com/fefejiro/FTC-HOLDING/actions/runs/31419736829) and [native run `31419736905`](https://github.com/fefejiro/FTC-HOLDING/actions/runs/31419736905) passed on the exact commit. The persisted lifecycle derives participants from a locked active canonical two-person conversation and enforces regional grants, idempotency, concurrency, expiry, cleanup, direct-role denial, and content-free audit. Signaling, SDP/ICE persistence, media, TURN, native call UI, managed deployment, Simulator, and device proof remain absent |
...
| Regional managed deployment | DEPLOYED STAGING VERIFIED at exact target `1ff6f64c7`: protected dry-runs `31424479269`/`31424590161` and deployments `31424695462`/`31424817202` passed for Canada/U.S. All 19 migrations, `peacepad-v2-api`, and public health/readiness boundaries are deployed; authenticated journeys and restoration remain pending |
