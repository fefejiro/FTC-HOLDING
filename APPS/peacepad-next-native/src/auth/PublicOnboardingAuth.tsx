...
const copy = {
  en: {
    ...
    newHere: "New to PeacePad?", apple: "Continue with Apple", confirmTitle: "Check your email",
    ...
    appleUnavailable: "Sign in with Apple is not available right now.", passwordHint: "Use at least 8 characters."
  },
  fr: {
    slides: [
    ...
    newHere: "Nouveau sur PeacePad?", apple: "Continuer avec Apple", confirmTitle: "Consultez votre courriel",
    ...
    appleUnavailable: "La connexion avec Apple est indisponible pour le moment.", passwordHint: "Utilisez au moins 8 caractères."
  },
  es: {
    slides: [
    ...
    newHere: "¿Eres nuevo en PeacePad?", apple: "Continuar con Apple", confirmTitle: "Revisa tu correo",
    ...
    appleUnavailable: "Iniciar sesión con Apple no está disponible ahora.", passwordHint: "Usa al menos 8 caracteres."
  }
} as const;
  ...
  if (!introComplete) {
    ...
    return (
      <View style={styles.intro}>
        <Image accessible={false} accessibilityIgnoresInvertColors source={slideImages[slide]} style={styles.hero} />
        <View style={styles.introCopy}>
          ...
          <LabButton label={slide === strings.slides.length - 1 ? strings.start : strings.next} onPress={() => {
            if (slide === strings.slides.length - 1) void finishIntro(); else setSlide((value) => value + 1);
          }} />
          {slide < strings.slides.length - 1 ? <Pressable accessibilityRole="button" onPress={() => void finishIntro()}><Text style={styles.link}>{strings.skip}</Text></Pressable> : null}
        </View>
      </View>
    );
  }
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.keyboard}>
      <ScrollView contentContainerStyle={styles.auth} keyboardShouldPersistTaps="handled">
        ...
        /> : null}
        {appleAvailable ? <Text style={styles.or}>{strings.or}</Text> : null}
