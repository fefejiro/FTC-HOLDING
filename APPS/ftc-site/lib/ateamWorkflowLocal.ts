...
const LOCAL_RUNS_KEY = "ateam-local-workflow-runs-v1";
const LOCAL_FALLBACK_EVENT = "ateam:local-fallback";
...
type WorkflowPayload<T> = T & { ok: true; localFallback?: boolean };
...
function emitLocalFallback(reason: string) {
  if (!isBrowser()) return;
...
function readStore(): LocalWorkflowStore {
  if (!isBrowser()) return { runs: [] };
  try {
    const raw = window.localStorage.getItem(LOCAL_RUNS_KEY);
    if (!raw) return { runs: [] };
    const parsed = JSON.parse(raw) as LocalWorkflowStore;
    return {
...
function writeStore(store: LocalWorkflowStore) {
  if (!isBrowser()) return;
  window.localStorage.setItem(LOCAL_RUNS_KEY, JSON.stringify(store));
}

function nextRunId() {
  return `wfr_local_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
}
...
function persistRun(nextRun: WorkflowRun) {
  const store = readStore();
  const index = store.runs.findIndex((run) => run.id === nextRun.id);
  if (index >= 0) {
    store.runs[index] = cloneJson(nextRun);
  } else {
    store.runs.unshift(cloneJson(nextRun));
  }
  store.runs.sort((left, right) => String(right.updatedTs || "").localeCompare(String(left.updatedTs || "")));
  writeStore(store);
...
function findRun(runId: string) {
  const store = readStore();
  const match = store.runs.find((run) => run.id === runId);
...
function createLocalRun(payload: {
  idea: string;
  category?: string;
  templateId?: string;
  ...
  const catalog = getCatalog();
  const template = getWorkflowTemplate(payload.templateId || "") as WorkflowTemplate | null;
  const category = normalizeWorkflowCategory(payload.category || template?.category || "", payload.idea);
  const intake = {
    goal: toText(payload.intake?.goal || template?.intake?.goal, 260),
    context: toText(payload.intake?.context || template?.intake?.context, 360),
    desiredOutput: toText(payload.intake?.desiredOutput || template?.intake?.desiredOutput, 180),
    constraints: toText(payload.intake?.constraints || template?.intake?.constraints, 260),
  ...
  plan = withTemplateId(plan, toText(template?.id || payload.templateId || "", 80));
  ...
  const baseRun: WorkflowRun = {
    ...
    phase: "analysis",
    state: "planning",
    requestedBy: "public_web_local",
    category,
    idea: toText(payload.idea, 1200),
    ...
    handoff: {},
    meta: {
      templateId: toText(template?.id || payload.templateId || "", 80),
...
function listLocalRuns(searchParams: URLSearchParams) {
  ...
  if (category && category !== "all") {
    runs = runs.filter((run) => String(run.category || "").toLowerCase() === category);
  }
  if (state && state !== "all") {
    runs = runs.filter((run) => String(run.state || "").toLowerCase() === state);
  }
  return {
    ok: true,
    localFallback: true,
    ...
    catalog: getCatalog(),
  } satisfies WorkflowPayload<{ runs: WorkflowRun[]; catalog: WorkflowCatalog }>;
}

function getLocalRun(runId: string) {
  const run = findRun(runId);
  if (!run) {
    throw new Error("ATEAM could not find that local fallback run anymore.");
  }
  return {
    ok: true,
    localFallback: true,
...
function updateLocalRunAnswers(runId: string, body: Record<string, unknown>) {
  const run = findRun(runId);
  if (!run) throw new Error("ATEAM could not find that local fallback run anymore.");
...
function approveLocalRun(runId: string, body: Record<string, unknown>) {
  const run = findRun(runId);
  if (!run) throw new Error("ATEAM could not find that local fallback run anymore.");
  ...
  if (gate === "brief" && decision === "regenerate") {
    ...
    const templateId = toText(run.plan?.editable?.templateId || run.meta?.templateId, 80);
    regenerated.plan = withTemplateId(regenerated.plan as WorkflowPlan, templateId);
    return {
      ok: true,
      localFallback: true,
      run: persistRun(regenerated),
      catalog: getCatalog(),
    } satisfies WorkflowPayload<{ run: WorkflowRun; catalog: WorkflowCatalog }>;
  }
  ...
  if (gate === "pack" && decision === "approved") {
    ...
    return {
      ok: true,
      localFallback: true,
      run: persistRun(completed),
      catalog: getCatalog(),
    } satisfies WorkflowPayload<{ run: WorkflowRun; catalog: WorkflowCatalog }>;
  }

  throw new Error("ATEAM local fallback could not process that approval action.");
}

function generateLocalPack(runId: string) {
  const run = findRun(runId);
  if (!run) throw new Error("ATEAM could not find that local fallback run anymore.");
...
export function shouldUseLocalWorkflowFallback(message = "", status = 0) {
  ...
  return (
    ...
    lower.includes("workflow service is not connected") ||
    lower.includes("unexpected response instead of the workflow api") ||
    lower.includes("unable to reach the ateam workflow service")
  );
}

export function getLocalWorkflowFallbackEventName() {
  return LOCAL_FALLBACK_EVENT;
}

export async function handleLocalWorkflowRequest<T = unknown>(path: string, init?: RequestInit): Promise<T> {
  emitLocalFallback("cloud_backend_paused");
  ...
  if (pathname === "/api/ateam/workflow/runs" && method === "GET") {
    return listLocalRuns(url.searchParams) as T;
  }

  if (pathname === "/api/ateam/workflow/runs" && method === "POST") {
    return createLocalRun({
      idea: toText(body.idea, 1200),
      category: toText(body.category, 80),
      templateId: toText(body.templateId, 80),
      intake: (body.intake || {}) as WorkflowIntake,
    }) as T;
  }

  const match = pathname.match(/^\/api\/ateam\/workflow\/runs\/([^/]+)(?:\/(answers|approve|generate-pack))?$/i);
  if (!match) {
    throw new Error("ATEAM local fallback does not recognize this workflow route.");
  }
  ...
  if (!action && method === "GET") {
    return getLocalRun(runId) as T;
  }
  if (action === "answers" && method === "POST") {
    return updateLocalRunAnswers(runId, body) as T;
  }
  if (action === "approve" && method === "POST") {
    return approveLocalRun(runId, body) as T;
  }
  if (action === "generate-pack" && method === "POST") {
    return generateLocalPack(runId) as T;
  }

  throw new Error("ATEAM local fallback could not handle this workflow request.");
}
