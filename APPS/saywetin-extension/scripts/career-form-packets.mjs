...
function main() {
  ...
  if (!quickApplyPath || !fs.existsSync(quickApplyPath)) {
    console.error(
      "Quick-apply JSON not found. Run career:batch first or pass a path.",
    );
    process.exit(1);
  }
  ...
  selected.forEach((job, idx) => {
    ...
    const qna = cfg.questionMappings
      .map((q) => {
    ...
    indexLines.push(
      `| ${idx + 1} | ${job.title} | ${job.company} | ${path.relative(ROOT, outPath).replace(/\\/g, "/")} |`,
    );
  });

  const indexPath = path.join(
    outDir,
    `index-${new Date().toISOString().replace(/[:.]/g, "-")}.md`,
  );
