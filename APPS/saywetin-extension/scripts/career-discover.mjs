...
const SERPAPI_QUERIES = [
  {
    q: "Agile Project Coordinator contract remote Canada",
    chips: "employment_type:CONTRACTOR",
  },
  {
    q: "Scrum Master contract remote Canada OR USA",
    chips: "employment_type:CONTRACTOR",
  },
  { q: "IT Project Coordinator ERP remote Canada", chips: "" },
  {
    q: "Business Systems Analyst WMS supply chain remote",
    chips: "employment_type:CONTRACTOR",
  },
  {
    q: "ERP Implementation Business Analyst remote Canada",
    chips: "employment_type:CONTRACTOR",
  },
  { q: "Supply Chain Systems Analyst remote Canada OR USA", chips: "" },
  { q: "Program Coordinator delivery lead Azure DevOps remote", chips: "" },
  {
    q: "LinkedIn Business Systems Analyst remote Canada contract",
    chips: "employment_type:CONTRACTOR",
  },
  {
    q: "Indeed Agile Project Coordinator remote Canada contract",
    chips: "employment_type:CONTRACTOR",
  },
];
...
const FALLBACK_LINKEDIN_QUERIES = [
  {
    q: "site:linkedin.com/jobs ERP Business Analyst remote Canada",
    chips: "employment_type:CONTRACTOR",
  },
  {
    q: "site:linkedin.com/jobs Scrum Master contract remote Canada",
    chips: "employment_type:CONTRACTOR",
  },
];
...
const FALLBACK_INDEED_QUERIES = [
  {
    q: "site:indeed.com Project Coordinator ERP remote contract",
    chips: "employment_type:CONTRACTOR",
  },
  {
    q: "site:indeed.ca Business Systems Analyst WMS remote",
    chips: "employment_type:CONTRACTOR",
  },
];
...
async function safeFetch(url, label) {
  try {
    const res = await fetch(url, {
      headers: {
        "User-Agent": "career-discover/2.0 (personal job search tool)",
      },
...
function detectEmploymentType(title, desc) {
  ...
  if (hay.includes("part-time") || hay.includes("part time"))
    return "Part-time";
  return "Full-time";
}
...
async function fetchSerpApi(queries, apiKey) {
  ...
  for (const { q, chips } of queries) {
    ...
    for (const j of data.jobs_results) {
      ...
      const highlights =
        j.job_highlights?.flatMap((h) => h.items || []).join(" ") || "";
    ...
    console.log(
      `[SerpAPI] "${q}" → ${data.jobs_results.length} results (${added} geo-matched)`,
    );
    await delay(600);
  }
...
async function fetchRemotive(searches) {
  ...
  for (const query of searches) {
    ...
    console.log(
      `[Remotive] "${query}" → ${data.jobs.length} results (${added} geo-matched)`,
    );
    await delay(300);
  }
...
function isRelevant(job, profile) {
  ...
  const hitCount = profile.coreKeywords.filter((k) =>
    blob.includes(normalize(k)),
  ).length;
  return hitCount >= 1;
}
...
async function main() {
  ...
  if (serpApiKey) {
    console.log(
      "Mode: SerpAPI — Google Jobs (aggregates LinkedIn, Indeed, Glassdoor, ZipRecruiter)\n",
    );
    ...
    if (linkedinHits < 1) {
      console.log(
        "[SerpAPI] No LinkedIn results yet. Running LinkedIn fallback pass...",
      );
    ...
    if (indeedHits < 1) {
      console.log(
        "[SerpAPI] No Indeed results yet. Running Indeed fallback pass...",
      );
    ...
    console.log(
      "  → For LinkedIn + Indeed results, sign up free at: https://serpapi.com\n",
    );
  ...
  if (final.length === 0) {
    ...
    if (!serpApiKey) {
      console.warn(
        "Set SERPAPI_KEY for LinkedIn + Indeed results: https://serpapi.com",
      );
    }
