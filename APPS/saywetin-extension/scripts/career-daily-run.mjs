...
function main() {
  ...
  if (runDiscover) {
    ...
    const discoverStatus = runNodeScript(
    ...
    if (discoverStatus === 0 && fs.existsSync(path.join(ROOT, jobsOut))) {
      ...
      console.warn(
        "[daily] Discovery failed or returned 0 jobs — falling back to URL ingest or sample data.",
      );
    }
  }
  ...
  if (!runDiscover || jobsInput === fallbackJobs) {
    if (ingestExists) {
      const status = runNodeScript(
        "scripts/career-ingest-urls.mjs",
        [urlsFile, jobsOut],
        true,
      );
      if (status === 0) {
  ...
  if (
    !fs.existsSync(path.join(ROOT, jobsInput)) &&
  ...
  if (!fs.existsSync(path.join(ROOT, jobsInput))) {
    console.error(
      "No jobs input available. Provide --urls file or a valid fallback jobs JSON file.",
    );
    process.exit(1);
  }
