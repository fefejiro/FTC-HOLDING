...
function extractRoleFamilyScores(jdText, roleFamilies) {
  ...
  const entries = Object.entries(roleFamilies).map(([familyName, cfg]) => {
    const hits = cfg.mustHaveKeywords.filter((k) => haystack.includes(normalize(k)));
...
function pickTopKeywords(matches, limit = 14) {
  return matches.filter((m) => m.found).slice(0, limit).map((m) => m.keyword);
}
...
function buildSummaryBullets(title, matchedKeywords, highlights) {
  ...
  const primary = selected.length > 0 ? selected.join(', ') : 'systems delivery, integration, and testing';
  ...
  return [
    ...
    highlights[0] ? `Recent example: ${highlights[0]}.` : 'Recent example: delivered cross-team implementation support in enterprise environments.',
  ];
}
...
function buildSkillsBullets(matchedKeywords) {
  ...
  if (chunks.length === 0) {
    return ['Systems analysis, Agile delivery, requirements management, integration testing, stakeholder communication'];
  }
...
function buildExperienceBullets(jdText, highlights) {
  ...
  if (lower.includes('azure devops')) {
    bullets.push('Managed backlog, sprint flow, and progress reporting in Azure DevOps to keep delivery transparent for business and technical stakeholders.');
  }

  if (lower.includes('scrum') || lower.includes('agile')) {
    bullets.push('Facilitated Agile ceremonies, removed blockers, and coordinated cross-functional teams to maintain sprint goals and release timelines.');
  }

  if (lower.includes('wms') || lower.includes('warehouse')) {
    bullets.push('Supported WMS and warehouse operations initiatives across requirements, testing, and go-live readiness in retail and logistics settings.');
  }

  if (lower.includes('erp') || lower.includes('edi') || lower.includes('integration')) {
    bullets.push('Coordinated ERP and downstream integration activities, including requirement definition, validation, and issue triage with vendors and internal teams.');
  }

  if (lower.includes('risk') || lower.includes('dependency')) {
    bullets.push('Tracked dependencies and escalated risks early, with concise status updates that improved decision speed and reduced delivery delays.');
  }
...
function renderMarkdownReport(data) {
  ...
  const roleRows = roleFamilyScores
    .map((r) => `| ${r.familyName} | ${r.score}% | ${r.hits.length}/${r.total} | ${r.hits.join(', ') || 'None'} |`)
    .join('\n');
...
function main() {
  ...
  if (!inputArg) {
    console.error('Usage: node scripts/job-hunt-pipeline.mjs <jd-file-path> [company-name]');
    process.exit(1);
  }

  const jdPath = path.isAbsolute(inputArg) ? inputArg : path.join(ROOT, inputArg);
  ...
  const roleFamilyScores = extractRoleFamilyScores(jdText, profile.roleFamilies);
  const topRoleFamily = roleFamilyScores[0]?.familyName || 'agile_project_delivery';
  const topKeywords = pickTopKeywords(keywordMatches, 18);
  const title = getFirstMatchingTitle(jdText, roleFamilyScores);

  const summaryBullets = buildSummaryBullets(title, topKeywords, profile.experienceHighlights);
  const skillsBullets = buildSkillsBullets(topKeywords);
  const experienceBullets = buildExperienceBullets(jdText, profile.experienceHighlights);
  const outreachMessage = buildOutreachMessage(title, companyName || 'Hiring Team');
