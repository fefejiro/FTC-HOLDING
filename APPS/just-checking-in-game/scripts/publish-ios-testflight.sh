...
echo "[JCI] iOS 1.1.0/build 4 publish run ${RUN_ID}"
...
grep -q 'iPhone: 4' "${UNITY_PROJECT}/ProjectSettings/ProjectSettings.asset" || fail "ProjectSettings is not iOS build 4"
...
echo "[JCI] Upload completed. This is not public availability. Confirm build 4 in App Store Connect, submit review with automatic release, then verify the public listing after approval."
