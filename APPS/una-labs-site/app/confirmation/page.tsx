...
function ConfirmationContent() {
  ...
  useEffect(() => {
    ...
    let plan = '';
    if (raw) {
      try { plan = JSON.parse(raw).plan ?? ''; } catch { /* ignore */ }
    }
    const labels: Record<string, string> = {
      starter: 'Starter', professional: 'Professional', agency: 'Agency', enterprise: 'Enterprise',
    };
    ...
    fetch(`${STRIPE_API_URL}/api/activate-project`, {
      ...
      body: JSON.stringify({ session_id: sessionId, intake: raw ? JSON.parse(raw) : null }),
    })
      ...
      <div className="min-h-screen bg-bg-offwhite flex items-center justify-center px-6">
        <div className="max-w-tight w-full bg-white rounded-2xl border border-border p-10 text-center shadow-sm">
          ...
          <p className="text-body text-tx-secondary mb-6">
            Your payment may have gone through but we couldn't activate your account automatically.
            Email <a href="mailto:hello@unalabs.cloud" className="text-brand-teal underline">hello@unalabs.cloud</a> and we'll sort it out within 24 hours.
          </p>
    ...
    <div className="min-h-screen bg-bg-offwhite flex items-center justify-center px-6">
      <div className="max-w-tight w-full bg-white rounded-2xl border border-border p-10 text-center shadow-sm">
        ...
        <div className="text-left flex flex-col gap-3 mb-8">
          {[
            ['Check your inbox', 'We sent a setup guide to your email. Follow it to get your first project live.'],
            ['Set up your workspace', 'Add your team, configure intake forms, and invite your first client.'],
            ['Explore the dashboard', 'See every project at a glance from your real-time dashboard.'],
          ].map(([title, desc], i) => (
            <div key={i} className="flex gap-3 p-4 bg-bg-offwhite rounded-xl">
