    ...
    <section className="overflow-hidden bg-white pt-14 pb-20 lg:pt-20 lg:pb-24">
      <div className="mx-auto grid max-w-content items-center gap-12 px-6 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
        <div className="max-w-xl">
          ...
          <h1 className="mt-5 text-display text-tx-heading">Bring the next good idea to life.</h1>
          <p className="mt-6 max-w-lg text-body-lg leading-relaxed text-tx-secondary">
            Tell us what you are trying to accomplish. We help shape it, build it, and leave you with a product people can actually use.
          </p>
          <div className="mt-8 flex flex-wrap gap-4">
            <Button href="/start" variant="primary" size="lg">Start Your Project</Button>
            <Button href="/demo" variant="ghost" size="lg">See Una Labs in action →</Button>
          </div>
          <p className="mt-4 text-caption text-tx-muted">A short first step. No technical brief required.</p>
        </div>
        ...
        <div className="relative min-h-[340px] rounded-[32px] border border-border bg-bg-offwhite p-5 shadow-sm sm:p-8" aria-label="Una Labs delivery journey">
          ...
          <div className="relative rounded-[24px] bg-[#0f1117] p-5 text-white shadow-xl sm:p-7">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <span className="text-xs font-semibold tracking-[0.18em] text-white/50">UNA LABS / LIVE PATH</span>
            ...
            <div className="flex items-center justify-between border-t border-white/10 pt-4 text-xs text-white/40">
              <span>Clear next step</span><span className="text-[#75d7c7]">{Math.round(((active + 1) / STATES.length) * 100)}% in view</span>
            </div>
          </div>
        </div>
      </div>
