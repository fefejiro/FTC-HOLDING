...
<head>
  ...
  <meta name="description" content="ATEAM turns one rough idea into a clear first path, visible work, and a client-ready handoff with Una Labs." />
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .contact-card label{display:grid;gap:8px;font-size:.92rem;color:var(--ink)}
    .contact-card input,.contact-card textarea{width:100%;border:1px solid var(--line);border-radius:16px;padding:12px 14px;font:inherit;background:rgba(255,255,255,.86);color:var(--ink)}
...
<body>
    ...
    <section class="layout">
      <div class="stack">
        ...
        <div id="contactCard" class="contact-card" hidden>
          <div>
            <p class="eyebrow">Project handoff</p>
            <h3 style="margin-bottom:6px">Send this straight to Una Labs.</h3>
            <p class="muted">ATEAM already attached the idea, output, and handoff pack. You only need to say where to reply.</p>
          </div>
          <div class="contact-grid">
            ...
            <label>Email
              <input id="contactEmail" type="email" placeholder="hello@company.com" />
            </label>
            <label>Timeline
              <select id="contactTimeline">
          ...
          <label>Optional notes
            <textarea id="contactNotes" placeholder="Anything else we should know about urgency, approvals, or constraints?"></textarea>
          </label>
          <div class="result-actions">
            <button id="contactSubmitBtn" class="btn btn-primary" type="button">Send ATEAM handoff</button>
          </div>
          <div id="contactError" class="error" hidden></div>
          <div id="contactSuccess" class="contact-success" hidden></div>
        </div>
        </div>
      </div>
      <aside class="stack">
  ...
  <script>
    ...
    const state={run:null,answers:{},service:"checking",supportsVoice:false,isListening:false,recognition:null,intakeStartedAt:Date.now(),leadSubmitted:false,contactOpen:false};
    ...
    function setContactMessage(id,msg){const el=$(id); if(!el){return;} if(!msg){el.hidden=true; el.textContent=""; return;} el.hidden=false; el.textContent=msg;}
    function buildProjectIdeaForIntake(run){if(!run){return "";} const lines=[]; if(text(run.idea)){lines.push("Idea: "+text(run.idea));} if(text(run.recommendedLane||run.brief&&run.brief.recommendedLane)){lines.push("Lane: "+laneLabel(run.recommendedLane||run.brief&&run.brief.recommendedLane));} if(text(run.brief&&run.brief.quickVerdict)){lines.push("Quick read: "+text(run.brief.quickVerdict));} if(text(run.brief&&run.brief.summary)){lines.push("Summary: "+text(run.brief.summary));} if(text(run.brief&&run.brief.primaryGoal)){lines.push("Primary goal: "+text(run.brief.primaryGoal));} if(text(run.brief&&run.brief.recommendedDirection)){lines.push("Recommended direction: "+text(run.brief.recommendedDirection));} if(text(run.artifacts&&run.artifacts.mockup&&run.artifacts.mockup.title)||text(run.artifacts&&run.artifacts.prototype&&run.artifacts.prototype.title)){lines.push("Output pack: "+[text(run.artifacts&&run.artifacts.mockup&&run.artifacts.mockup.title),text(run.artifacts&&run.artifacts.prototype&&run.artifacts.prototype.title),text(run.artifacts&&run.artifacts.doc&&run.artifacts.doc.title)].filter(Boolean).join("; "));} return lines.join("\\n").trim();}
    function showContactCard(){const card=$("contactCard"); if(!card){return;} state.contactOpen=true; card.hidden=false; card.scrollIntoView({behavior:"smooth",block:"start"});}
    ...
    async function submitLead(){if(!state.run){return;} const email=text($("contactEmail")&&$("contactEmail").value); if(!email){setContactMessage("contactError","Add an email so Una Labs knows where to reply."); return;} const button=$("contactSubmitBtn"); if(button){button.disabled=true; button.textContent="Sending...";} setContactMessage("contactError",""); setContactMessage("contactSuccess",""); try{const handoff=handoffPayload(); const response=await req("/api/intake",{method:"POST",body:JSON.stringify({name:text($("contactName")&&$("contactName").value),email,projectName:text(handoff.brief&&handoff.brief.title)||"ATEAM handoff",projectType:text(handoff.recommendedLane)||text(handoff.categoryLabel)||"Not sure yet",projectIdea:buildProjectIdeaForIntake(state.run),budgetRange:text($("contactBudget")&&$("contactBudget").value)||"not-sure-yet",timeline:text($("contactTimeline")&&$("contactTimeline").value),notes:text($("contactNotes")&&$("contactNotes").value),companyWebsite:"",startedAt:state.intakeStartedAt,ateamWorkflow:handoff})}); state.leadSubmitted=true; render(); setContactMessage("contactSuccess","Request received. Reference: "+text(response&&response.requestId)+". Una Labs now has the ATEAM handoff and will reply from the attached pack.");}catch(err){setContactMessage("contactError",err instanceof Error?err.message:"Unable to send the ATEAM handoff right now.");}finally{if(button){button.disabled=false; button.textContent="Send ATEAM handoff";}}}
    ...
    async function boot(){render(); setupVoice(); try{await req("/api/ateam/workflow/runs?limit=1"); state.service="ready"; $("serviceState").className="state"; $("serviceState").textContent="ATEAM workflow service is live";}catch{state.service="unavailable"; $("serviceState").className="state offline"; $("serviceState").textContent="ATEAM workflow service is unavailable"; setBox("noticeBox","The cloud workflow service is still warming up. You can review the flow and try again in a moment.","notice");} const runId=new URL(location.href).searchParams.get("run"); if(runId){try{await loadRun(runId);}catch(err){setBox("errorBox",err instanceof Error?err.message:"Unable to load the requested run.","error");}}}
    $("startBtn").addEventListener("click",start); $("buildBtn").addEventListener("click",buildPack); $("resetBtn").addEventListener("click",reset); $("continueBtn").addEventListener("click",cont); $("restartBtn").addEventListener("click",reset); $("voiceBtn").addEventListener("click",toggleVoiceCapture); $("contactSubmitBtn").addEventListener("click",submitLead); boot();
  </script>
</body>
</html>`;
}
